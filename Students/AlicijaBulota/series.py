...
def fibonacci(n):
    """Fibonacci function to return nth value in series using recursion."""
...
def lucas(n):
    """function to find nth value for Lucas Numbers."""
    if not isinstance(n, int):
        raise ValueError
    if n < 0:
        return None
...
def sum_series(n, opt_par0=0, opt_par1=1):
    """function to find nth value in summed integer sequence."""
    if not isinstance(n, int):
        raise ValueError
    if n < 0:
        return None
